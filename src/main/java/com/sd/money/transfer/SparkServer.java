...
import com.sd.money.transfer.utils.Bank;
import com.sd.money.transfer.utils.JsonUtils;
import com.sd.money.transfer.utils.PaginationParams;
...
final class SparkServer {
    ...
    private static void initPartyRoutes() {
        // http://localhost:8080/parties?limit=10
        // http://localhost:8080/parties?page=2&limit=20
        Spark.get("/bank", (req, res) -> {
        ...
        // http://localhost:8080/parties/1
        Spark.get("/bank/:id", (req, res) -> {
            final Repository<Party> repository = Bank.getInstance().getContext().getPartyRepository();
            return JsonUtils.make().toJson(findById(Party.class, repository, req));
        });

        // http://localhost:8080/parties/1/accounts
        Spark.get("/bank/:id/accounts", (req, res) -> {
            final Repository<Party> repository = Bank.getInstance().getContext().getPartyRepository();
    ...
    private static void initTransactionRoutes() {
        // http://localhost:8080/transactions?limit=100
